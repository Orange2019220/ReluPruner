...
def replace_layers(model, i, indexes, layers):
    ...
    :param model: 待修改的模型序列
    :param i: 当前层索引
    :param indexes: 需要替换的层索引列表
    :param layers: 替换用的新层列表
    :return: 替换后的层对象
    """
    if i in indexes:
...
def prune_vgg16_conv_layer(model, layer_index, filter_index, use_cuda=False):
    ...
    :param model: 待剪枝的VGG16模型
    :param layer_index: 需要剪枝的卷积层索引（特征提取层中的位置）
    :param filter_index: 需要移除的过滤器索引
    :param use_cuda: 是否使用CUDA加速（默认False）
    :return: 剪枝后的VGG16模型
    """
    # 获取指定卷积层及其后续最近的卷积层
